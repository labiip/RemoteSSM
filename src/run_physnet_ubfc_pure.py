"""PhysNet 同协议复现 (UBFC+PURE 联合训练, subject-independent).

与 RemoteSSM-v3 主实验完全相同的协议 (表/4.2 中 "same protocol" 对照):
  - 同一数据管道: 32px face ROI, 缓存 cardio_v5_bvphr, seq 300
  - 同一数据划分: train=UBFC 1-34 + PURE 101-106 (40 subj),
                  val  =UBFC 35-39 + PURE 107-108,
                  test =UBFC 40-49 / PURE 109-110 (各自独立评估)
  - 同一评测口径: 10s 窗, step 30 帧 (VAL_STEP=30), 0.7-3.0 Hz,
                  BVP 衍生 HR 标签, soft-argmax FFT(BVP) 提取 HR,
                  指标 MAE/RMSE/Pearson/Acc<3/5/8
训练目标忠实 PhysNet 原论文 (Yu et al., BMVC 2019): 负 Pearson 波形监督
(trend-consistency)。不使用 RemoteSSM-v3 的频域/带外/HR 数值监督。
优化器按原论文 Adam, lr=1e-3, 采用通用 warmup+cosine 调度, AMP=False。

用法 (服务器):
  CUDA_VISIBLE_DEVICES=X nohup python run_physnet_ubfc_pure.py > physnet_ubfc_pure.out 2>&1 &
"""
import os
import sys
import math
...
OUT_DIR = os.path.join(PROJ, "outputs", "physnet_ubfc_pure")
TAG = "physnet_ubfc_pure"
...
def train_model():
    ...
    logging.info(f"PhysNet-3DCNN | Params: {n_params:.2f}M | Device: {device}")
...
def evaluate(model_path, test_subjects, name):
    ...
    print(f"\n[{name}] PhysNet (same protocol, step-30)")
...
if __name__ == "__main__":
    train_model()
    print("\n" + "=" * 56)
    print("  PhysNet test report (same protocol)")
    print("=" * 56)
