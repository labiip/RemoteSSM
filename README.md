...
├── checkpoints/                       27 个实验权重（按实验名分子目录，与 logs/ 中的日志一一对应）
├── logs/                              80 个日志（53 个评测/训练 *.out + 27 个训练 *_train.log）
├── requirements.txt
└── README.md
```

> 论文正文在 Overleaf 上维护，未包含在本仓库中。
...
```bash
# ① 主实验: UBFC+PURE 联合训练（产生 UBFC 0.87 / PURE 0.31 的权重）
...
# ③ MMPD 主行: 从①的权重微调（论文表 1/2/4 的 13.69）
...
评测脚本按 `outputs/<实验名>/<权重文件名>` 查找权重，首次使用请先建立目录：

```bash
cp -r checkpoints/* src/outputs/
```

> 27 个实验权重均已随仓库分发（`checkpoints/<实验名>/`），与下表的「权重」列一一对应。

| 论文位置 | 数值 | 评测脚本 | 日志 | 权重 |
| --- | --- | --- | --- | --- |
| 表 1/2：UBFC | 0.87 | `_eval_step30.py` | `eval_step30.out` | `ubfc_pure` |
| 表 1/2：PURE | 0.31 | `_eval_step30.py` | `eval_step30.out` | `ubfc_pure` |
| 表 1/2/4：MMPD | **13.69** | `eval_new_ablations_step30.py` | `eval_new_ablations_step30.out` | `mmpd_dual_pt_v30` |
| §4.4 UBFC-only 对照 | 1.04 | `_eval_step30.py` | `eval_step30.out` | `ubfc_only` |
| 表 3：full | 0.87 | `eval_ubfc_step30.py` | `eval_ubfc_step30.out` | `ubfc_pure` |
| 表 3：实数 SSM | 1.30 | `eval_new_ablations_step30.py` | `eval_new_ablations_step30.out` | `ubfc_ablate_real_ssm` |
| 表 3：w/o 带外约束 | 0.97 | `eval_ubfc_step30.py` | `eval_ubfc_step30.out` | `ubfc_ablate_no_outband` |
| 表 3：w/o COB | 0.59 | `eval_ubfc_step30.py` | `eval_ubfc_step30.out` | `ubfc_ablate_no_cob` |
| 表 3：标量 HR 监督 | 23.28 | `eval_ubfc_step30.py` | `eval_ubfc_step30.out` | `ubfc_ablate_bpm_supervision` |
| 表 4：仅面部, 复数 | 25.33 | `_eval_faceonly30.py` | `evalfc_step30.out` | `mmpd_facemain_pt` |
| 表 4：仅面部, 实数 | 28.23 | `_eval_real30.py` | `evalfc_step30.out` | `mmpd_real_pt` |
| 表 4：双路, 实数 | 13.74 | `run_mmpd_dual_real_pt.py`（自带评测） | `mmpd_dual_real_pt.out` | `mmpd_dual_real_pt` |
| 表 4：双路, w/o 残差 | 13.31 | `run_mmpd_dual_nores_pt.py`（自带评测） | `mmpd_dual_nores_pt.out` | `mmpd_dual_nores_pt` |
| 表 4：双路, w/o COB | 17.60 | `eval_new_ablations_step30.py` | `eval_new_ablations_step30.out` | `mmpd_dual_nocob_pt` |
| 跨域：UBFC-only → MMPD | 19.72 | `_verify_step30.py` | `verify_step30.out` | `ubfc_only` |
| 跨域：联合 → MMPD | 20.76 | `_eval_step30.py` | `eval_step30.out` | `ubfc_pure` |
| 基线 PhysNet (ours) | 1.63 / 1.13 | `run_physnet_ubfc_pure_r2.py` | `physnet_ubfc_pure_r2.out` | `physnet_ubfc_pure_r2` |
| 基线 PhysMamba | 2.90 / 7.04 | （临时脚本，未归档） | `physmamba_ubfc_pure.out` | `physmamba_ubfc_pure` |
| 基线 RhythmMamba | 7.03 / 3.38 | （临时脚本，未归档） | `rhythmmamba_ubfc_pure.out` | `rhythmmamba_ubfc_pure` |

补充说明：

- `mmpd_dual_pt`（13.55，见 `eval_step30.out`）是 MMPD 主行的**旧选点版本**（训练时按 val_step=300 选点），已被 `mmpd_dual_pt_v30`（13.69，val_step=30 统一口径）取代，保留仅供追溯；其自身训练日志 `mmpd_dual_pt.out` 末尾的 step=300 自评测为 14.43。
- UBFC 消融另有 `w/o 残差`（0.70）与 OOD 组（`eval_leave_step30.out`），未进正文表格。
- `ubfc_quick` / `ubfc_pure_quick` 为调试用的快速短跑，非论文结果。
...
- **文件命名**：源码与权重统一使用项目命名——`main.py`（主脚本）、`model.py`（模型定义 `RemoteSSM`）、`dataset.py`、`train.py`、`evaluate.py`，权重文件名前缀为 `remotessm_`；各实验的名称保留在 `checkpoints/` 与 `logs/` 的目录/文件名中，与第 5 节表格一一对应。
...
- **未随仓库分发的内容**：原始数据集（公开、体积大）、离线缓存（可由第 3 节重建）、数据集外部的第三方权重、论文图源与绘图脚本/绘图数据、第三方基线（PhysNet / PhysMamba / RhythmMamba）的原始仓库与预训练权重、训练过程中的 `*_final.pth` / `*_resume.pth`。
